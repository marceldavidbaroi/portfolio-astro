...
| **Work Directory**<br>`/work`<br>`src/pages/work.astro` | **Platform Catalog & Proof of Work**<br>• Engineering Leads<br>• Hiring Managers | • Comprehensive catalog of 12+ production platforms and architectural builds.<br>• CV-card layout gives each project the rigor of an executive resume bullet.<br>• Categorized tech pills (`Vue 3.5`, `Quasar`, `PostgreSQL`) allow fast ATS keyword matching. | • **No category filter tabs** (e.g. `All`, `Enterprise SaaS`, `Mobile & IoT`, `FinTech`). If a recruiter is sourcing specifically for Vue or PostgreSQL, they must scroll through all 12 projects.<br>• No search bar to filter by technology keyword. | **MEDIUM** |
| **Case Study Deep Dive**<br>`/work/[slug]`<br>`src/pages/work/[slug].astro` | **Technical Competency & System Design**<br>• VP of Engineering<br>• Lead Architects | • **World-class engineering case study structure.**<br>• Executive Overview with quantified KPI pills.<br>• Architectural Decisions Matrix (`Challenge` → `Decision` with trade-offs → `Outcome`).<br>• Value Proposition Canvas (Severe Pains Before vs. Architectural Gains After).<br>• Clean interactive Table of Contents. | • Missing a sticky **"Discuss this system / Contact"** CTA at the bottom of each case study.<br>• Needs an estimated **"Read Time"** badge (e.g., *4 min read*) so recruiters can budget their review time.<br>• Ensure all proprietary systems clearly indicate IP status if source code is private. | **LOW**<br>*(Already Gold Standard)* |
| **Master Profile**<br>`/about`<br>`src/pages/about.astro` | **Candidate Identity & Holistic Fit**<br>• Recruiters<br>• Hiring Managers<br>• Culture Screeners | • Metrics ribbon (`3.91 CGPA`, `100% RLS Isolation`, `10+ Shipped Apps`).<br>• High-depth principles (Architecture-first AI synthesis, Zero-trust relational data).<br>• Comprehensive coverage of skills, credentials, and personal pursuits. | • **Cognitive overload**: Page is over 1,600 lines long with dense text blocks.<br>• **Information hierarchy is inverted**: Philosophy appears *before* Technical Skills. Recruiters need the Tech Stack first.<br>• **Redundancy**: Directly duplicates standalone pages (`/credentials`, `/skills`, `/education`).<br>• **High school (HSC) prominent in the hero**: Distracts from senior enterprise engineering positioning. | **HIGH** |
| **Skills Matrix**<br>`/skills`<br>`src/pages/skills/index.astro` | **ATS Keyword Verification & Depth**<br>• Technical Recruiters<br>• Lead Engineers | • Contextual descriptions for each skill rather than a meaningless cloud of logo pills.<br>• Logical categorization into 4 architectural pillars.<br>• Emphasizes enterprise architecture, RLS, and SemVer registries. | • **Route redundancy**: Duplicate route exists at `/about/skills.astro`. Keep one single canonical route.<br>• Does not differentiate between **Primary Production Stack** (hire me for this today) vs. **Secondary Exposure** (prior tools). | **MEDIUM** |
...
* **Interactive Domain & Stack Filter on `/work`**:
  * Introduce instant filter tabs at the top of the project catalog:
    `[All (12)]` `[Enterprise SaaS]` `[Mobile & IoT]` `[Vue/Quasar]` `[PostgreSQL]`
  * Empowers recruiters to isolate relevant projects in under 2 seconds.
...
* **Estimated Reading Time on Long-Form Case Studies**:
  * Add `⏱️ 4 min read` alongside an executive TL;DR summary at the top of comprehensive case studies.

---

## Part 6: Global Strategic Implementation Priority Matrix

| Phase | Upgrade Task | Target File(s) | Impacted Persona | Estimated Effort | Recruiter Priority |
| :---: | :--- | :--- | :--- | :---: | :---: |
| **Phase 1** | **Add Persistent "Resume (PDF)" button to Global Header** | `src/layouts/BaseLayout.astro` | Technical Recruiters, Sourcers | 10 mins | **CRITICAL** |
| **Phase 1** | **OpenGraph & Twitter Card Social Preview Meta Tags** | `src/layouts/BaseLayout.astro`<br>`public/og-preview.png` | Sourcing Teams on Slack / LinkedIn | 30 mins | **HIGH** |
| **Phase 1** | **Add 1-Click "Copy Email" with Tooltip Feedback** | `src/layouts/BaseLayout.astro`<br>`src/pages/about.astro` | All Visitors | 15 mins | **HIGH** |
| **Phase 2** | **Reorganize `about.astro` to 9-Stage Recruiter Flow** | `src/pages/about.astro`<br>`src/components/about/*` | Hiring Managers, Recruiters | 2 hours | **HIGH** |
| **Phase 2** | **Add Interactive Category / Stack Filter to `/work`** | `src/pages/work.astro` | Technical Recruiters, Leads | 45 mins | **HIGH** |
| **Phase 3** | **Link Homepage Experience Timeline directly to Case Studies** | `src/pages/index.astro` | Engineering Managers, Leads | 20 mins | **MEDIUM** |
| **Phase 3** | **De-emphasize High School (HSC) & Highlight B.Sc. + Thesis** | `src/pages/index.astro`<br>`src/pages/about.astro` | International Recruiters | 25 mins | **HIGH** |
| **Phase 4** | **Add Architecture Flowcharts (Mermaid.js) to Private IP Projects** | `src/content/projects/*.md` | VP of Eng, Principal Architects | 1.5 hours | **HIGH** |
| **Phase 4** | **Add Quick Keyboard Command Menu (`Cmd + K`)** | `src/components/CommandMenu.astro` | Fellow Developers, Product Leads | 1.5 hours | **MEDIUM** |
